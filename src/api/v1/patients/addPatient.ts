...
export const addOne = async (req: Request, res: Response) => {
    const {body} = req
    const idNumberPatient : PatientModel = await PatientModel.findOne({where: {identificationNumber: body.identificationNumber}})

    if (idNumberPatient) {
        res.status(409).json({
            message: 'Patient with this identification number already existed'
        })
    } else {
